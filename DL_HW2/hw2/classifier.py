...
class Classifier(nn.Module, ABC):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # ====== YOUR CODE: ======
        z = self.model(x) #should i skip the forward?
        # ========================
    ...
    def predict_proba_scores(self, z: Tensor) -> Tensor:
        ...
        # ====== YOUR CODE: ======
        # N = z.shape[0]
        # prob_tensor = torch.zeros(z.shape)
        # for i in range(N):
        #     row_sum = torch.sum(z[i])
        #     prob_tensor[i] = z[i]/row_sum
...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # print("N= ", N)
        output = torch.zeros(N, dtype=torch.int)
        # print("initial output is: ", output)
        # for i in range(N):
        output = torch.argmax(y_proba, dim=1)
        # print("output[i], for i=", i, "is: ", output[i])
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        # print("positive_class is: ", self.positive_class)
        # print("y_proba = ", y_proba)
        # print("the sahpe of y_proba is: ", y_proba.shape)
        # print("treshold is: ", self.threshold)
        N = y_proba.shape[0]
        # print("N= ", N)
        output = torch.zeros(N, dtype=torch.int)
        # print("initial output is: ", output)
        for i in range(N):
            output[i] = (y_proba[i,self.positive_class] >= self.threshold)
        #  print(output, output.shape)
        return output
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    x1_boundries = torch.arange(x[:, 0].min(), x[:, 0].max(), dx)
    x2_boundries = torch.arange(x[:, 1].min(), x[:, 1].max(), dx)
    x1_grid, x2_grid = torch.meshgrid(x1_boundries, x2_boundries) #, indexing='ij')

    init_y = torch.cat([x1_grid.flatten().reshape(1,-1), x2_grid.flatten().reshape(1,-1)],dim=0).t()
    y_hat = classifier.classify(init_y).reshape(x1_grid.shape)
    # ========================
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    y_score = classifier.predict_proba(x)[:,1].detach()
    fpr, tpr, thresh = roc_curve(y, y_score)

    # calculate the point closest to (0,1)
    x_dist = torch.pow(Tensor(fpr),2)
    y_dist = torch.pow(1-Tensor(tpr),2)
    # distance_list = torch.sqrt(y_dist+x_dist)
    distance_list = y_dist + x_dist
    optimal_thresh_idx = torch.argmin(distance_list)
