...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        dx = dout * (self.grad_cache['sigmoid'] * (1-self.grad_cache['sigmoid']))
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        x_y = x.gather(1, y.unsqueeze(1))
        log_sigma_ek = torch.log(torch.sum(torch.exp(x), dim=1)).unsqueeze(1)
        loss = torch.sum(-x_y + log_sigma_ek)/N
    ...
    def backward(self, dout=1.0):
        ...
        d_x_y[range(x.shape[0]), y] = -1
        d_log_sigma_ek = torch.exp(x) / (torch.sum(torch.exp(x), dim=1, keepdim=True))
        dx = dout * (d_x_y + d_log_sigma_ek) / N
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if (self.training_mode):
            mask = torch.where(torch.rand_like(x) < (1 - self.p), 1., 0.) / (1-self.p)
    ...
    def __init__(
        ...
        **kw,
        ...
        for out_features in hidden_features:
            layers.append(Linear(temp_in_features, out_features))
            layers.append(activation_func())
            if dropout > 0:
                layers.append(Dropout(dropout))
            temp_in_features = out_features

        layers.append(Linear(temp_in_features, num_classes))
