...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += p * self.reg
            self.velocity[self.i] = self.momentum * self.velocity[self.i] - self.learn_rate * dp
            p += self.velocity[self.i]
            self.i += 1
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.i = 0
        self.rms = [torch.zeros_like(dp) for _, dp in self.params]
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += p * self.reg
            self.rms[self.i] = self.decay * self.rms[self.i] + (1-self.decay) * (dp ** 2)
            p -= self.learn_rate * (dp / torch.sqrt(self.rms[self.i] + self.eps))
            
            # update index
            self.i += 1
            if self.i >= len(self.rms): 
                self.i = 0
