    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # get training and test results for the epoch
            train_result = self.train_epoch(dl_train, **kw, verbose=verbose)
            test_result = self.test_epoch(dl_test, **kw, verbose=verbose)

            # calc and eppend the averaged training loss and accuracy for the epoch 
            avg_train_loss = sum(train_result.losses)/len(train_result.losses)
            train_loss.append(avg_train_loss)
            train_acc.append(train_result.accuracy)

            # calc and eppend the averaged test loss and accuracy for the epoch 
            avg_test_loss = sum(test_result.losses)/len(test_result.losses)
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # forward pass: The flow of information from the input to the output 
        y_pred = self.model(X)
        ...
        return BatchResult(batch_loss.item(), int(num_correct))
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        return BatchResult(batch_loss.detach().item(), int(num_correct))
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # forward pass
        model_out = self.model.forward(X.view(X.shape[0], -1))
