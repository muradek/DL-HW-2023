...
def train_epoch(model, training_data, optimizer, device):
    ...
    for src_seq,trg_seq in tqdm(training_data, mininterval=2, desc=desc, leave=False):
        # prepare data
        # forward
        optimizer.zero_grad()

        src_seq = src_seq.transpose(1,0)
        trg_seq = trg_seq.transpose(1,0)
        gold = trg_seq
...
def train(model, training_data, validation_data, optimizer, device, epochs):
    ''' Start training '''

    log_train_file = os.path.join('logs', 'train.log')
    log_valid_file = os.path.join('logs', 'valid.log')
